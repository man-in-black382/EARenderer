...
#pragma mark - Setters
    ...
    void GLSLSurfelLighting::setLight(const PointLight &light) {
        ...
        glUniform1f(uniformByNameCRC32(uint32_constant<ctcrc32("uPointLight.constant")>).location(), light.attenuation.constant);
        glUniform1f(uniformByNameCRC32(uint32_constant<ctcrc32("uPointLight.linear")>).location(), light.attenuation.linear);
        glUniform1f(uniformByNameCRC32(uint32_constant<ctcrc32("uPointLight.quadratic")>).location(), light.attenuation.quadratic);
        glUniform1f(uniformByNameCRC32(uint32_constant<ctcrc32("uPointLight.area")>).location(), light.area());
    ...
    void GLSLSurfelLighting::setLight(const DirectionalLight &light) {
        ...
        glUniform1f(uniformByNameCRC32(uint32_constant<ctcrc32("uDirectionalLight.area")>).location(), light.area());
    ...
    void GLSLSurfelLighting::setProbePositions(const GLFloat3BufferTexture<glm::vec3> &positions) {
        setBufferTexture(uint32_constant<ctcrc32("uProbePositions")>, positions);
    }
