...
namespace EARenderer {
    ...
    struct SceneGBuffer {
        std::shared_ptr<GLIntegerTexture2D < GLTexture::Integer::RGBA32UI>> materialData;
        ...
        SceneGBuffer(const Size2D &resolution)
                :
                materialData(std::make_shared<GLIntegerTexture2D < GLTexture::Integer::RGBA32UI>>

        (resolution)),
        HiZBuffer(std::make_shared<GLFloatTexture2D < GLTexture::Float::R32F>>
        (resolution)),
