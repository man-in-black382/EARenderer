...
public:
    ...
    //! Reserve constructor.
    FbxArray(const int pCapacity) : mSize(0), mCapacity(0), mArray(NULL) {
        if (pCapacity > 0) Reserve(pCapacity);
    }

    //! Copy constructor.
    FbxArray(const FbxArray &pArray) : mSize(0), mCapacity(0), mArray(NULL) {
        *this = pArray;
    }
