...
private:
    enum EMask {
        eActive = 1 << 0, eMinX = 1 << 1, eMinY = 1 << 2, eMinZ = 1 << 3, eMaxX = 1 << 4, eMaxY = 1 << 5, eMaxZ =
        1 << 6, eAll = eMinX | eMinY | eMinZ | eMaxX | eMaxY | eMaxZ
    };
...
private:
    ...
    public:
        enum EMask {
            eHasNothing = 0, eHasPreRotM = 1 << 0, eHasPostRotM = 1 << 1
        };
    ...
    enum EMask {
        eHasNothing = 0, eHasRotOffset = 1 << 0, eHasRotPivot = 1 << 1, eHasScaleOffset = 1 << 2, eHasScalePivot =
        1 << 3
    };
