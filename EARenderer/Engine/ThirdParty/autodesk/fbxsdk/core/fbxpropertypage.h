...
private:
    FbxPropertyInfo(const char *pName, FbxPropertyPage *pTypeInfo)
            : mRef(0), mName(pName), mType(eFbxUndefined), mTypeInfo(pTypeInfo), mUserTag(0), mUserData(0), mFilter(0) {
    ...
    FbxPropertyInfo(const char *pName, EFbxType pType)
            : mRef(0), mName(pName), mType(pType), mTypeInfo(0), mUserTag(0), mUserData(0), mFilter(0) {
        for (int i = 0; i < eValueCount; i++) {
            mMinMaxValue[i] = 0;
        }
    }
...
protected:
    FbxPropertyPage(FbxPropertyPage *pInstanceOf = 0)
            : mInstanceOf(0), mDataPtr(0), mPropNextId(0) {
